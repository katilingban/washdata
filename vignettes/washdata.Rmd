...
1. Get appropriate maps for the city to be surveyed. The types of maps that will be appropriate for this urban sampling approach are:
    ...
    a) Map data files[^16] that delineate the boundaries of the whole city and each of the three levels of the sampling frame[^17];
    
    b) High resolution gridded population maps[^18]; and,
    
    c) Map data on roads, buildings, residential areas, landuse and places of interest for the city[^19].
...
[^7]: It is interesting to note that based on the WSUP questionnaires, pregnant women are identified separately but are classified within the grouping of those with disability
[^8]:The questionnaire uses a 4 category scale i.e., satisfied, somewhat satisfied, somewhat dissatisfied, dissatisfied
[^9]: WSUP uses the Progress out of Poverty Index (PPI)
[^10]: In some cities, these settlements are categorised or named as slums or squatters areas and can be formally or informally recognised by authorities and/or city planners
[^11]: Especially true if the mapping of the low-income/poor areas are fixed specifically on administrative boundaries and not at more localised areas
[^12]: See Lanata, C.F. & Black, R.E., 1991. Lot quality assurance sampling techniques in health surveys in developing countries: advantages and current constraints. World Health Statistics Quarterly, 44(3), pp.133–139
[^13]: See Lanata, C.F. et al., 1988. Lot quality assurance sampling in health monitoring. The Lancet, 1(8577), pp.122–123 and Robertson, S.E. & Valadez, J.J., 2006. Global review of health care surveys using lot quality assurance sampling (LQAS), 1984– 2004. Social Science & Medicine, 63(6), pp.1648–1660.
[^14]: Based on previous work done by Valid International on application of LQAS on reporting water and sanitation indicators recommended by the Joint Monitoring Programme (JMP)
[^15]: Some modifications to these steps may need to be done on a country by country basis particularly with regard to how the groupings of the spatial units are organised and sub-divided. Otherwise, the general approach is the same on any setting.
[^16]: There are many formats for map files. In general, the most common and most accessible formats are keyhole markup language (KML) files which are the standard output format produced by Google Maps/Google Earth and ESRI Shapefiles.
[^17]: These days,there are various available online repositories of map data files for most countries including cities. We most commonly use the repository maintained by Robert Hijmans at http://www.gadm.org and the one maintained by UN OCHA called Humanitarian Data Exchange (https://data.humdata.org). Organisations such as the World Food Programme also maintain their own repositories but most of their maps are also made available through the Humanitarian Data Exchange.
[^18]: The best openly available sources for this map data are from the WorldPop project (<http://www.worldpop.org.uk>) and the Global Rural-Urban Mapping Project or GRUMP (<http://sedac.ciesin.columbia.edu/data/collection/grump-v1>).
[^19]: For most cities,this can be available from OpenStreetMap or OSM (<http://www.openstreetmap.org>). Google Earth can also be used to view satellite imagery of the city.
